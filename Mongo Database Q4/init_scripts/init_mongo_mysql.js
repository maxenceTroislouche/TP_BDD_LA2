...
async function creerNotesParHashtag() {
    let retours = await db.retourclient.find({}).toArray();
    ...
    for (let retour of retours) {
        let nbNotes = Math.floor(Math.random() * (hashtagsPossible.length))
        let hashtags = [...hashtagsPossible];
        for (let i = 0; i < nbNotes; i++) {
            let idxHashtags = Math.floor(Math.random() * (hashtags.length - 1));
            let hashtag = hashtags[idxHashtags];
            hashtags.splice(idxHashtags, 1);  // On retire le hashtag choisi de la liste
            dataToInsert.push({
                idRetour: retour._id,
                note: Math.floor(Math.random() * 5),
                hashtag: hashtag
    ...
    await db.noteparhashtag.insertMany(dataToInsert);
}
